...
class KubernetesDependencySolver:
    ...
    def _find_resources_by_labels(
        self, target_service_name: str, labels: dict
    ) -> List[Resource]:
        matches = []
                    ...
                    for path in paths:
                        ...
                        if service_name:
                            ...
                            for tgt in self.resources:
                                if (
                                    tgt.service_name == "K8S_SERVICE"
                                    and tgt.name.endswith(f"/{service_name}")
                                ):
                                    dependencies.append(
                                        Dependency(
            ...
            if res.service_name in [
                ...
                for vol in volumes:
                    if "configMap" in vol:
                        ...
                        for tgt in self.resources:
                            if (
                                tgt.service_name == "K8S_CONFIGMAP"
                                and tgt.name.endswith(f"/{cm_name}")
                            ):
                                dependencies.append(
                                    Dependency(
                                        source=f"{res.resource_type}.{res.name}",
                        ...
                        for tgt in self.resources:
                            if tgt.service_name == "K8S_SECRET" and tgt.name.endswith(
                                f"/{secret_name}"
                            ):
                                dependencies.append(
                                    Dependency(
                ...
                for container in containers:
                    ...
                    for env in env_from:
                        if "configMapRef" in env:
                            ...
                            for tgt in self.resources:
                                if (
                                    tgt.service_name == "K8S_SECRET"
                                    and tgt.name.endswith(f"/{secret_name}")
                                ):
                                    dependencies.append(
                                        Dependency(
                                            source=f"{res.resource_type}.{res.name}",
