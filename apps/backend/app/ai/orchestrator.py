...
from app.ai.prompts import explain, security, cost
...
class AIOrchestrator:
    ...
    def explain_infrastructure(self, summary: InfraSummary, query: str = "Explain this infrastructure.") -> str:
    ...
    def review_security(self, summary: InfraSummary, query: str = "What are the biggest security risks?") -> str:
        context = self._summary_to_json(summary)
        prompt = security.build_security_prompt(context, query)
        return self.client.generate(security.SYSTEM_PROMPT, prompt)
        
    def review_cost(self, summary: InfraSummary, query: str = "How can I optimize costs here?") -> str:
        context = self._summary_to_json(summary)
