    ...
    for root, dirs, files in os.walk(TESTS_DIR):
        if "expected.json" in files:
            ...
            try:
                ...
                actual_risk_signatures = [(r.severity, r.category) for r in actual_risks]
                expected_risk_signatures = [(r["severity"], r["category"]) for r in expected_risks]
                
                # Check expectations
                missing_risks = [r for r in expected_risk_signatures if r not in actual_risk_signatures]
                extra_risks = [r for r in actual_risk_signatures if r not in expected_risk_signatures]
                ...
                elapsed_time = time.time() - start_time
                print(f"[  FAILED  ] {test_name} ({elapsed_time:.2f}s) - Exception: {e}")
                failed_tests.append(test_name)
